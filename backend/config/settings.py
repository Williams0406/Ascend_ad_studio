...
ALLOWED_HOSTS = [
    x.strip()
    for x in os.getenv("ALLOWED_HOSTS", "localhost,127.0.0.1").split(",")
    if x.strip()
]
...
if os.getenv("DATABASE_URL"):
    DATABASES = {
        "default": dj_database_url.config(
            ...
            ssl_require=os.getenv("DATABASE_SSL_REQUIRED", "False").lower() == "true",
        )
    }
...
STORAGES = {
    "default": {
        "BACKEND": (
            "config.storage.CloudinaryMediaStorage"
            if USE_CLOUDINARY
...
SERVE_MEDIA_FILES = (
    os.getenv("SERVE_MEDIA_FILES", "True" if DEBUG else "False").lower() == "true"
)
...
CSRF_TRUSTED_ORIGINS = [
    x.strip()
    for x in os.getenv("CSRF_TRUSTED_ORIGINS", "http://localhost:3000").split(",")
    if x.strip()
]
...
ENABLE_PROVIDER_KEY_REMOTE_VALIDATION = (
    os.getenv("ENABLE_PROVIDER_KEY_REMOTE_VALIDATION", "True").lower() == "true"
)
...
GEMINI_PROMPT_COMPOSER_MODEL = os.getenv(
    "GEMINI_PROMPT_COMPOSER_MODEL",
    "gemini-2.5-pro",
)

GEMINI_PROMPT_COMPOSER_TIMEOUT = int(
    os.getenv(
...
# Número de reintentos posteriores al primer intento.
GEMINI_PROMPT_COMPOSER_MAX_RETRIES = int(
    os.getenv(
...
GEMINI_COMPOSED_PROMPT_MIN_LENGTH = int(
    os.getenv(
...
FAL_OUTPUT_DOWNLOAD_TIMEOUT = int(
    os.getenv(
        "FAL_OUTPUT_DOWNLOAD_TIMEOUT",
        "120",
    )
)
...
GEMINI_BRAND_INTELLIGENCE_MODEL = os.getenv(
    "GEMINI_BRAND_INTELLIGENCE_MODEL",
    "gemini-2.5-pro",
)

GEMINI_BRAND_INTELLIGENCE_TIMEOUT = int(
    os.getenv(
...
GEMINI_TEMPLATE_ANALYSIS_MODEL = os.getenv(
    "GEMINI_TEMPLATE_ANALYSIS_MODEL",
    "gemini-2.5-pro",
)

GEMINI_CONCEPT_PLANNER_MODEL = os.getenv(
    "GEMINI_CONCEPT_PLANNER_MODEL",
    "gemini-2.5-pro",
)

GEMINI_CONCEPT_PLANNER_TIMEOUT = int(
    os.getenv(
...
GEMINI_TEMPLATE_ANALYSIS_TIMEOUT = int(
    os.getenv(
