...
const emptyProduct = {
  ...
  image_assets: [], benefits: [], features: [], is_active: true,
};
...
function PencilIcon() {
  return <svg viewBox="0 0 24 24" aria-hidden="true"><path d="m14.7 5.3 4 4M4 20l3.8-.8L19 8a2.1 2.1 0 0 0-3-3L4.8 16.2 4 20Z"/></svg>;
}
...
export default function ProductsPage() {
  ...
  const selectedAsset = assets.find(asset => String(asset.id) === String(form.main_image_asset));
  function update(key, value) { setForm(current => ({ ...current, [key]: value })); }
  function flash(type, text) { setMessage({ type, text }); window.setTimeout(() => setMessage({ type: '', text: '' }), 4500); }

  function openCreate() {
    setEditing(null); setForm({...emptyProduct}); setSelected(null); setFormOpen(true);
    requestAnimationFrame(() => document.querySelector('.product-form-shell')?.scrollIntoView({ behavior: 'smooth', block: 'start' }));
  }

  function openEdit(product, event) {
    event?.stopPropagation();
    const values = Object.fromEntries(Object.keys(emptyProduct).map(key => [key, product[key] ?? emptyProduct[key]]));
    ...
    if (values.main_image_asset && !values.image_assets.includes(String(values.main_image_asset))) values.image_assets.unshift(String(values.main_image_asset));
    setEditing(product); setForm(values); setSelected(null); setFormOpen(true);
    requestAnimationFrame(() => document.querySelector('.product-form-shell')?.scrollIntoView({ behavior: 'smooth', block: 'start' }));
  }

  function closeForm() { setEditing(null); setForm({...emptyProduct}); setFormOpen(false); }
  function toggleImage(assetId) {
    const id=String(assetId);const selected=form.image_assets.map(String);const exists=selected.includes(id);
    const next=exists?selected.filter(value=>value!==id):[...selected,id];
    update('image_assets',next);
    if(!exists&&!form.main_image_asset)update('main_image_asset',id);
    if(exists&&String(form.main_image_asset)===id)update('main_image_asset',next[0]||'');
  }
  ...
  async function submit(event) {
    ...
    try {
      ...
      if(payload.main_image_asset&&!payload.image_assets.includes(String(payload.main_image_asset)))payload.image_assets.unshift(String(payload.main_image_asset));
      const saved = await api(editing?`/studio/products/${editing.id}/`:'/studio/products/', { method: editing?'PATCH':'POST', body: JSON.stringify(payload) });
      setProducts(current => editing?current.map(product=>product.id===saved.id?saved:product):[saved, ...current]); closeForm(); setSelected(saved); flash('success', editing?'Producto actualizado correctamente.':'Producto registrado correctamente.');
  ...
  return <><Nav privateNav/><main className="container products-studio">
    <header className="products-header"><div><span className="eyebrow">Catálogo comercial</span><h1>Productos</h1><p>Organiza la información comercial y visual que alimenta tus campañas.</p></div><button className={`btn product-create-toggle ${formOpen ? 'open' : ''}`} onClick={formOpen?closeForm:openCreate}><span>{formOpen ? '×' : '+'}</span>{formOpen ? 'Cerrar formulario' : 'Registrar producto'}</button></header>
    {message.text && <div className={`brand-toast ${message.type}`}>{message.text}</div>}

    <section className={`product-form-shell ${formOpen ? 'open' : ''}`} aria-hidden={!formOpen}><form onSubmit={submit}><div className="product-form-head"><div><span>{editing?'Editando registro':'Nuevo registro'}</span><h2>{editing?'Editar producto':'Ficha de producto'}</h2></div><p>{editing?'Actualiza la información comercial y la selección visual del producto.':'Completa los datos que la IA necesita para crear mensajes precisos y piezas relevantes.'}</p></div>
      ...
      <aside className="product-image-picker"><div className="form-section-title"><b>04</b><div><h3>Galería del producto</h3><p>Selecciona una o varias imágenes y define la portada.</p></div></div><div className="chosen-product-image">{selectedAsset ? <img src={selectedAsset.file_url} alt={selectedAsset.name}/> : <div><span>Imagen principal</span><small>Selecciona al menos una imagen</small></div>}</div><div className="product-image-count"><span>{form.image_assets.length} seleccionada{form.image_assets.length===1?'':'s'}</span><small>{selectedAsset?`Principal · ${selectedAsset.name}`:'Sin imagen principal'}</small></div><div className="asset-choice-grid product-multi-assets">{assets.map(asset => {const chosen=form.image_assets.map(String).includes(String(asset.id));const main=String(form.main_image_asset)===String(asset.id);return <div className={`${chosen?'selected':''} ${main?'main':''}`} key={asset.id}><button type="button" onClick={()=>toggleImage(asset.id)} aria-pressed={chosen}><img src={asset.file_url} alt=""/><span>{asset.name}</span><i>{chosen?'✓':'+'}</i></button>{chosen&&<button type="button" className="set-main-image" onClick={()=>update('main_image_asset',String(asset.id))}>{main?'Principal':'Usar como principal'}</button>}</div>})}</div>{!assets.length && <div className="picker-empty">No hay imágenes en BrandAsset.<Link href="/brand-kit">Subir recursos</Link></div>}<label className="product-active"><span><b>Producto activo</b><small>Disponible para nuevas campañas</small></span><input type="checkbox" checked={form.is_active} onChange={e => update('is_active',e.target.checked)}/><i/></label></aside></div>
      <div className="product-form-actions"><button type="button" className="btn secondary" onClick={closeForm}>Cancelar</button><button className="btn" disabled={busy}>{busy ? 'Guardando…' : editing?'Guardar cambios':'Guardar producto'}</button></div>
    </form></section>
